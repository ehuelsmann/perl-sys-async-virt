...
add the prefix C<virDomain>.  The functions in this module may need to be
prefixed with either C<virConnect> or C<virNode>.  An important difference with
the C API is that this API only lists the C<INPUT> and C<INPUT|OUTPUT (as
input)> arguments for its functions.  The C<OUTPUT> and C<INPUT|OUTPUT (as
output)> arguments will be returned in the C<on_reply> event.
