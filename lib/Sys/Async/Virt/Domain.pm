...
class Sys::Async::Virt::Domain v0.1.10;
...
async sub _managed_direct_migrate($self, $dest_client, $params, $flags) {
    my $src_client = $self->{client};
    my $remote = $self->{remote};
    die $log->error(q{migrate() with 'dest_client' does not support PEER2PEER an TUNNELLED flags})
    ...
    my $params_feature = $remote->DRV_FEATURE_MIGRATION_PARAMS;
    die $log->error(q{Destination host missing feature MIGRATION_PARAMS; }
                    . q{older migration protocols not supported})
        if (await $dest_client->_supports_feature( $params_feature ));

    my $offline_feature = $remote->DRV_FEATURE_MIGRATION_OFFLINE;
    die $log->error(q{Source host does not support offline migration})
        if (await $src_client->_supports_feature( $offline_feature ));
    ...
    my $protection = 0;
    my $protection_feature = $remote->DRV_FEATURE_CHANGE_PROTECTION;
    if (await $src_client->_supports_feature( $protection_feature )) {
        die $log->error(q{Cannot enforce change protection})
            if ($flags & MIGRATE_CHANGE_PROTECTION);
        $protection = MIGRATE_CHANGE_PROTECTION;
    }
    $flags &= ~MIGRATE_CHANGE_PROTECTION;

    ### start migrating; virdomainmigrateversion3full
    my $rv;
    $rv = await $src_client->_call(
        $remote->PROC_DOMAIN_MIGRATE_BEGIN3_PARAMS,
        { dom => $self->{id}, params => $params,
          flags => ($flags | $protection) });

    my ($dom_xml, $src_cookie) = $rv->@{ qw( xml cookie ) };
    my ($dest_cookie, $dest_uri);
    try {
        # replace the MIGRATE_PARAM_DEST_XML parameter with $dom_xml
        try {
            $rv = await $self->get_state;
            if ($rv->{'state'} == PAUSED
                and not ($flags & MIGRATE_PAUSED)) {
        ...
        $rv = await $dest_client->_call(
            ...
            { cookie_in => $src_cookie, params => $params, flags => $dest_flags });
        ($dest_cookie, $dest_uri) = $rv->@{ qw( cookie_out uri_out ) };
    }
    catch ($e) { }
    finally {
    }
...
async sub _managed_p2p_migrate($self, $dest_libvirt_uri, $params, $flags) {
...
async sub _unmanaged_direct_migrate($self, $params, $flags) {
    die $log->error(q{migrate() requires MIGRATE_PARAM_URI when called without both 'dest_client' and 'dest_client_uri'})
        if (not get_typed_parameter( $params, MIGRATE_PARAM_URI ));
}

async sub migrate($self, :$dest_client = undef, :$dest_libvirt_uri = undef,
                           :$params = [], :$flags = 0) {
    ...
    my $params_feature = $self->{remote}->DRV_FEATURE_MIGRATION_PARAMS;
    die $log->error(q{Source host missing feature MIGRATION_PARAMS; }
                    . q{older migration protocols not supported})
        if (await $self->{client}->_supports_feature( $params_feature ));

    if ($dest_client) {
        return await $self->_managed_direct_migrate($dest_client);
    }
...
=item STATS_BLOCK_SUFFIX_LIMITS_REQUEST_ALIGNMENT

=item STATS_BLOCK_SUFFIX_LIMITS_DISCARD_MAX

=item STATS_BLOCK_SUFFIX_LIMITS_DISCARD_ALIGNMENT

=item STATS_BLOCK_SUFFIX_LIMITS_WRITE_ZEROES_MAX

=item STATS_BLOCK_SUFFIX_LIMITS_WRITE_ZEROES_ALIGNMENT

=item STATS_BLOCK_SUFFIX_LIMITS_TRANSFER_OPTIMAL

=item STATS_BLOCK_SUFFIX_LIMITS_TRANSFER_MAX

=item STATS_BLOCK_SUFFIX_LIMITS_TRANSFER_HW_MAX

=item STATS_BLOCK_SUFFIX_LIMITS_IOV_MAX

=item STATS_BLOCK_SUFFIX_LIMITS_IOV_HW_MAX

=item STATS_BLOCK_SUFFIX_LIMITS_MEMORY_ALIGNMENT_MINIMAL

=item STATS_BLOCK_SUFFIX_LIMITS_MEMORY_ALIGNMENT_OPTIMAL

=item STATS_BLOCK_SUFFIX_TIMED_GROUP_COUNT

=item STATS_BLOCK_SUFFIX_TIMED_GROUP_PREFIX

=item STATS_BLOCK_SUFFIX_TIMED_GROUP_SUFFIX_INTERVAL

=item STATS_BLOCK_SUFFIX_TIMED_GROUP_SUFFIX_RD_LATENCY_MIN

=item STATS_BLOCK_SUFFIX_TIMED_GROUP_SUFFIX_RD_LATENCY_MAX

=item STATS_BLOCK_SUFFIX_TIMED_GROUP_SUFFIX_RD_LATENCY_AVG

=item STATS_BLOCK_SUFFIX_TIMED_GROUP_SUFFIX_WR_LATENCY_MIN

=item STATS_BLOCK_SUFFIX_TIMED_GROUP_SUFFIX_WR_LATENCY_MAX

=item STATS_BLOCK_SUFFIX_TIMED_GROUP_SUFFIX_WR_LATENCY_AVG

=item STATS_BLOCK_SUFFIX_TIMED_GROUP_SUFFIX_ZONE_APPEND_LATENCY_MIN

=item STATS_BLOCK_SUFFIX_TIMED_GROUP_SUFFIX_ZONE_APPEND_LATENCY_MAX

=item STATS_BLOCK_SUFFIX_TIMED_GROUP_SUFFIX_ZONE_APPEND_LATENCY_AVG

=item STATS_BLOCK_SUFFIX_TIMED_GROUP_SUFFIX_FLUSH_LATENCY_MIN

=item STATS_BLOCK_SUFFIX_TIMED_GROUP_SUFFIX_FLUSH_LATENCY_MAX

=item STATS_BLOCK_SUFFIX_TIMED_GROUP_SUFFIX_FLUSH_LATENCY_AVG

=item STATS_BLOCK_SUFFIX_TIMED_GROUP_SUFFIX_RD_QUEUE_DEPTH_AVG
...
=item EVENT_SUSPENDED_GUEST_SHUTDOWN
...
=item BACKUP_BEGIN_PRESERVE_SHUTDOWN_DOMAIN
